...
import { getLaunchStatus, startLaunch } from '../services/openclaw-launch-service.js';
...
export const startLaunchWorkflowTool = createTool({
  ...
  execute: async ({ idea }, context) => {
    const threadId = getThreadIdFromToolContext(context);
    const run = await startLaunch(idea, undefined, threadId);
