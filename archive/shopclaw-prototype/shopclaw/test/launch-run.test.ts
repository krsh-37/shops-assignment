...
import { InMemoryLaunchRunStore, InvalidFounderPromptError, type LaunchRunStore, startLaunchRun } from '../mastra/tools/launch-run.ts';

test('startLaunchRun creates a run-scoped record for a valid founder prompt', async () => {
  const store = new InMemoryLaunchRunStore();

  const record = await startLaunchRun('I want to sell custom socks delivered in 10 minutes', store, () => new Date('2025-03-23T10:00:00.000Z'));
...
test('startLaunchRun trims surrounding whitespace from a founder prompt', async () => {
  ...
  const record = await startLaunchRun('  I want to sell custom socks  ', store, () => new Date('2025-03-23T10:00:00.000Z'));
...
test('startLaunchRun rejects an empty founder prompt without creating a record', async () => {
  ...
  assert.equal(await store.get('missing-run'), null);
});
...
test('startLaunchRun does not report success when persistence fails', async () => {
  ...
  await assert.rejects(startLaunchRun('I want to sell custom socks', store, () => new Date('2025-03-23T10:00:00.000Z')), /disk unavailable/);
});
